...
| Single-file delivery | `dist/blockhawk.html` and identical `index.html`, approximately 815 KiB. Rendering library, models, textures, subset interface fonts, UI, synthesized audio and third-party notices are embedded. Offline `file://` launch and deployment pass, with zero external asset requests. |
